...
namespace BusicoTheme\Inc\Classes;
...
class Busico_Main
{
    ...
    public function pagination()
    {
        ...
        echo '<div class="col-12"><div class="busico-navigation">';
    ...
    public function busico_theContentWithComment()
    {
    ...
    public function busico_breadcrumb_bridge()
    {
        $busico = get_option('busico');

        if (isset($busico['breadcrumb_on'])) :
            if ($busico['breadcrumb_on'] == true) :
                $this->busico_get_the_breadcrumbs();
            endif;
        else :
            $this->busico_get_the_breadcrumbs();
        endif;
     ...
     * @return breadcrumb
     */
    public function busico_get_the_breadcrumbs()
    {
        $busico = get_option('busico');
        $title = $this->generateBreadCrumbTitle();

        if(is_single()){
            $busico_blog_image = $busico['blog_bredcrumb_img']['url'];
        }else{
            $busico_blog_image = $busico['blog_bredcrumb_single_img']['url'];
        };
        
        $output = '<div class="blog-breadcrumb" style="background-image: url('. $busico_blog_image .')">
        ...
        $output .= $this->busico_breadcrumbs();
        $output .= '
                        </div>
                    </div>
                </div>
    ...
    public function generateBreadCrumbTitle()
    {
        ...
        if (is_home() || is_front_page()) {
            $title = isset($busico['bp_title']) ? esc_html($busico['bp_title']) : esc_html__('Our Blog', 'busico');
        } elseif ('case-study' == busico_get_archive_post_type()) {
            $title = isset($busico['cs_title']) ? esc_html($busico['cs_title']) : esc_html__('Case Study', 'busico');
        } elseif ('job' == busico_get_archive_post_type()) {
            $title = isset($busico['job_title']) ? esc_html($busico['job_title']) : esc_html__(' Current job openings', 'busico');
        } elseif (is_page()) {
            $title = get_the_title();
        } elseif (is_single()) {
            $title = get_the_title();
        }elseif (function_exists('is_shop') && is_shop()) {
            $title = isset($busico['shop_title']) ? esc_html($busico['shop_title']) : esc_html__('Our Products', 'busico');
        } elseif (is_archive()) {
            $title = get_the_archive_title();
        } elseif (is_search()) {
            $title = esc_html__('Search results for: ', 'busico') . get_search_query();
        }elseif(is_404(  )){
            $title = isset($busico['404_title']) ? esc_html($busico['shop_title']) : esc_html__('404', 'busico');
        }

        return $title;
    }

    public static function busico_breadcrumbs()
    {

        $busico = get_option('busico');

        $sepOpt = (isset($busico['breadcrumb_sep']) ? $busico['breadcrumb_sep'] : '<i class="fa fa-angle-right"></i>');

        /* === OPTIONS === */
        $text['home'] = esc_html__('Home', 'busico'); // text for the 'Home' link
        $text['shop'] = esc_html__('Shop', 'busico'); // text for the 'Shop' link
        $text['category'] = esc_html__('Archive by Category "%s"', 'busico'); // text for a category page
        $text['search'] = esc_html__('Search Results for "%s" Query', 'busico'); // text for a search results page
        $text['tag'] = esc_html__('Posts Tagged "%s"', 'busico'); // text for a tag page
        $text['author'] = esc_html__('Articles Posted by %s', 'busico'); // text for an author page
        $text['404'] = esc_html__('Error 404', 'busico'); // text for the 404 page
        $text['page'] = esc_html__('Page %s', 'busico'); // text 'Page N'
        $text['cpage'] = esc_html__('Comment Page %s', 'busico'); // text 'Comment Page N'
